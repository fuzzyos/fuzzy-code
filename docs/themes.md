> fuzzy can create themes. Ask it to build one for your setup.
...
Fuzzy loads themes from:

- Built-in: `dark`, `light`
- Global: `~/.fuzzy/agent/themes/*.json`
- Project: `.fuzzy/themes/*.json`
- Packages: `themes/` directories or `fuzzy.themes` entries in `package.json`
...
On first run, fuzzy detects your terminal background and defaults to `dark` or `light`.
...
```bash
mkdir -p ~/.fuzzy/agent/themes
vim ~/.fuzzy/agent/themes/my-theme.json
```
...
```json
{
  "$schema": "https://raw.githubusercontent.com/fuzzyos/fuzzy/main/packages/fuzzy-code/src/modes/interactive/theme/theme-schema.json",
  "name": "my-theme",
  "vars": {
    "primary": "#00aaff",
    "secondary": 242
...
**Hot reload:** When you edit the currently active custom theme file, fuzzy reloads it automatically for immediate visual feedback.
...
Fuzzy uses 24-bit RGB colors. Most modern terminals support this (iTerm2, Kitty, WezTerm, Windows Terminal, VS Code). For older terminals with only 256-color support, fuzzy falls back to the nearest approximation.
