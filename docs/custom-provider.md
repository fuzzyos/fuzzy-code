...
Extensions can register custom model providers via `fuzzy.registerProvider()`. This enables:
...
export default function (fuzzy: ExtensionAPI) {
  ...
  fuzzy.registerProvider("my-provider", {
    name: "My Provider",
...
The extension factory can also be `async`. For dynamic model discovery, fetch and register models in the factory instead of `session_start`. fuzzy waits for the factory before startup continues, so the provider is available during interactive startup and to `fuzzy --list-models`.
...
Use `fuzzy.unregisterProvider(name)` to remove a provider that was previously registered via `fuzzy.registerProvider(name, ...)`:
...
// Later, remove it
fuzzy.unregisterProvider("my-llm");
```
...
models: [{
  ...
  // ...
  reasoning: true,
  thinkingLevelMap: {              // map fuzzy levels to provider values; null hides unsupported levels
    minimal: null,
    low: null,
    medium: null,
    high: "default",
...
```typescript
fuzzy.registerProvider("custom-api", {
  baseUrl: "https://api.example.com",
  apiKey: "$MY_API_KEY",
...
```typescript
import type { OAuthCredentials, OAuthLoginCallbacks } from "@fuzzyos/fuzzy-ai";
...
Credentials are persisted in `~/.fuzzy/agent/auth.json`:
...
- [anthropic.ts](https://github.com/fuzzyos/fuzzyos/blob/main/packages/fuzzy-ai/src/providers/anthropic.ts) - Anthropic Messages API
- [mistral.ts](https://github.com/fuzzyos/fuzzyos/blob/main/packages/fuzzy-ai/src/providers/mistral.ts) - Mistral Conversations API
- [openai-completions.ts](https://github.com/fuzzyos/fuzzyos/blob/main/packages/fuzzy-ai/src/providers/openai-completions.ts) - OpenAI Chat Completions
- [openai-responses.ts](https://github.com/fuzzyos/fuzzyos/blob/main/packages/fuzzy-ai/src/providers/openai-responses.ts) - OpenAI Responses API
- [google.ts](https://github.com/fuzzyos/fuzzyos/blob/main/packages/fuzzy-ai/src/providers/google.ts) - Google Generative AI
- [amazon-bedrock.ts](https://github.com/fuzzyos/fuzzyos/blob/main/packages/fuzzy-ai/src/providers/amazon-bedrock.ts) - AWS Bedrock
...
When a request exceeds the model's context window, fuzzy can recover automatically by compacting the conversation and retrying. This recovery only kicks in if fuzzy recognizes the failure as an overflow.

Detection runs on the finalized assistant message:

- `stopReason === "error"`
- `errorMessage` matches one of fuzzy's known overflow patterns (see [`packages/fuzzy-ai/src/utils/overflow.ts`](https://github.com/fuzzyos/fuzzyos/blob/main/packages/fuzzy-ai/src/utils/overflow.ts))

If your provider returns overflow errors with a message fuzzy does not recognize, normalize the error from the same extension that registers the provider. Use a `message_end` handler to rewrite the assistant message so its `errorMessage` starts with a phrase fuzzy recognizes. The generic fallback `context_length_exceeded` is the safest choice.
...
export default function (fuzzy: ExtensionAPI) {
  ...
  fuzzy.on("message_end", (event, ctx) => {
...
`message_end` runs before fuzzy tracks the assistant message for auto-compaction, so the rewritten `errorMessage` is what fuzzy checks. With this in place, fuzzy will:
...
- Match a provider-specific pattern, not fuzzy's generic overflow patterns. Rewriting rate-limit or throttling errors (`rate limit`, `too many requests`) would falsely trigger compaction instead of fuzzy's normal retry-with-backoff path.
...
Test your provider against the same test suites used by built-in providers. Copy and adapt these test files from [packages/fuzzy-ai/test/](https://github.com/fuzzyos/fuzzyos/tree/main/packages/fuzzy-ai/test):
...
interface ProviderModelConfig {
  ...
  /** Maps fuzzy thinking levels to provider/model-specific values; null marks a level unsupported. */
