> fuzzy can create TUI components. Ask it to build one for your use case.

# TUI Components

Extensions and custom tools can render custom TUI components for interactive user interfaces. This page covers the component system and available building blocks.

**Source:** [`@fuzzyos/fuzzy-tui`](https://github.com/fuzzyos/fuzzyos/tree/main/packages/tui)
...
```typescript
import { CURSOR_MARKER, type Component, type Focusable } from "@fuzzyos/fuzzy-tui";
...
The cursor remains hidden by default. This keeps the fake cursor rendering, while still positioning the hardware cursor for terminals that track IME candidate windows with hidden cursors. Some terminals require a visible hardware cursor for IME positioning; enable it with `showHardwareCursor`, `setShowHardwareCursor(true)`, or `FUZZY_HARDWARE_CURSOR=1`. The `Editor` and `Input` built-in components already implement this interface.
...
```typescript
import { Container, type Focusable, Input } from "@fuzzyos/fuzzy-tui";
...
**In custom tools** via `fuzzy.ui.custom()`:

```typescript
async execute(toolCallId, params, onUpdate, ctx, signal) {
  const handle = fuzzy.ui.custom(myComponent);
  // ...
  handle.close();
}
```
...
Import from `@fuzzyos/fuzzy-tui`:

```typescript
import { Text, Box, Container, Spacer, Markdown } from "@fuzzyos/fuzzy-tui";
```
...
```typescript
import { matchesKey, Key } from "@fuzzyos/fuzzy-tui";
...
```typescript
import { visibleWidth, truncateToWidth } from "@fuzzyos/fuzzy-tui";
...
```typescript
import { getMarkdownTheme } from "@fuzzyos/fuzzy-code";
import { Markdown } from "@fuzzyos/fuzzy-tui";
...
Set `FUZZY_TUI_WRITE_LOG` to capture the raw ANSI stream written to stdout.

```bash
FUZZY_TUI_WRITE_LOG=/tmp/tui-ansi.log npx tsx packages/fuzzy-tui/test/chat-simple.ts
```
...
For letting users pick from a list of options. Use `SelectList` from `@fuzzyos/fuzzy-tui` with `DynamicBorder` for framing.

```typescript
import type { ExtensionAPI } from "@fuzzyos/fuzzy-code";
import { DynamicBorder } from "@fuzzyos/fuzzy-code";
import { Container, type SelectItem, SelectList, Text } from "@fuzzyos/fuzzy-tui";
...
```typescript
import { BorderedLoader } from "@fuzzyos/fuzzy-code";

fuzzy.registerCommand("fetch", {
  handler: async (_args, ctx) => {
    const result = await ctx.ui.custom<string | null>((tui, theme, _kb, done) => {
...
For toggling multiple settings. Use `SettingsList` from `@fuzzyos/fuzzy-tui` with `getSettingsListTheme()`.

```typescript
import { getSettingsListTheme } from "@fuzzyos/fuzzy-code";
import { Container, type SettingItem, SettingsList, Text } from "@fuzzyos/fuzzy-tui";

fuzzy.registerCommand("settings", {
  handler: async (_args, ctx) => {
...
Customize the inline working indicator shown while fuzzy is streaming a response.
...
// Restore fuzzy's default spinner
ctx.ui.setWorkingIndicator();
```
...
import { matchesKey, truncateToWidth } from "@fuzzyos/fuzzy-tui";
