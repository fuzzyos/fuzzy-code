...
```
~/.fuzzy/agent/sessions/--<path>--/<timestamp>_<uuid>.jsonl
```
...
Sessions can be removed by deleting their `.jsonl` files under `~/.fuzzy/agent/sessions/`.

Fuzzy also supports deleting sessions interactively from `/resume` (select a session and press `Ctrl+D`, then confirm). When available, fuzzy uses the `trash` CLI to avoid permanent deletion.
...
Source on GitHub ([fuzzyos](https://github.com/fuzzyos/fuzzyos)):
...
### Base Message Types (from fuzzy-ai)
...
- `fromHook`: `true` if generated by an extension, `false`/`undefined` if fuzzy-generated (legacy field name)
...
Session metadata (e.g., user-defined display name). Set via `/name` command or `fuzzy.setSessionName()` in extensions.
