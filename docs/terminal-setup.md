...
Fuzzy uses the [Kitty keyboard protocol](https://sw.kovidgoyal.net/kitty/keyboard-protocol/) for reliable modifier key detection. Most modern terminals support this protocol, but some require configuration.
...
Fuzzy enables enhanced key reporting when available. If Terminal.app still sends plain Return for `Shift+Enter`, fuzzy uses a local macOS modifier fallback to treat that Return as `Shift+Enter`.

This fallback only works when fuzzy runs on the same Mac as Terminal.app. It cannot detect the local keyboard over remote SSH.
...
On WSL, WezTerm may require a visible hardware cursor for IME candidate window positioning. If CJK IME candidates do not follow the text cursor, set `FUZZY_HARDWARE_CURSOR=1` before running fuzzy or set `showHardwareCursor` to `true` in settings.
...
If you want the hardware cursor visible, set `FUZZY_HARDWARE_CURSOR=1` before running fuzzy (disabled by default for compatibility).
