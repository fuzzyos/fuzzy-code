> fuzzy can create extensions. Ask it to build one for your use case.

# Extensions

Extensions are TypeScript modules that extend fuzzy's behavior. They can subscribe to lifecycle events, register custom tools callable by the LLM, add commands, and more.

> **Placement for /reload:** Put extensions in `~/.fuzzy/agent/extensions/` (global) or `.fuzzy/extensions/` (project-local) for auto-discovery. Use `fuzzy -e ./path.ts` only for quick tests. Extensions in auto-discovered locations can be hot-reloaded with `/reload`.

**Key capabilities:**
- **Custom tools** - Register tools the LLM can call via `fuzzy.registerTool()`
- **Event interception** - Block or modify tool calls, inject context, customize compaction
- **User interaction** - Prompt users via `ctx.ui` (select, confirm, input, notify)
- **Custom UI components** - Full TUI components with keyboard input via `ctx.ui.custom()` for complex interactions
- **Custom commands** - Register commands like `/mycommand` via `fuzzy.registerCommand()`
- **Session persistence** - Store state that survives restarts via `fuzzy.appendEntry()`
...
Create `~/.fuzzy/agent/extensions/my-extension.ts`:
...
| Location | Scope |
|----------|-------|
| `~/.fuzzy/agent/extensions/*.ts` | Global (all projects) |
| `~/.fuzzy/agent/extensions/*/index.ts` | Global (subdirectory) |
| `.fuzzy/extensions/*.ts` | Project-local |
| `.fuzzy/extensions/*/index.ts` | Project-local (subdirectory) |
...
To share extensions via npm or git as fuzzy packages, see [packages.md](packages.md).

## Available Imports

| Package | Purpose |
|---------|---------|
| `@fuzzyos/fuzzy-code` | Extension types (`ExtensionAPI`, `ExtensionContext`, events) |
| `typebox` | Schema definitions for tool parameters |
| `@fuzzyos/fuzzy-ai` | AI utilities (`StringEnum` for Google-compatible enums) |
| `@fuzzyos/fuzzy-tui` | TUI components for custom rendering |

npm dependencies work too. Add a `package.json` next to your extension (or in a parent directory), run `npm install`, and imports from `node_modules/` are resolved automatically.

For distributed fuzzy packages installed with `fuzzy install` (npm or git), runtime deps must be in `dependencies`. Package installation uses production installs (`npm install --omit=dev`) by default, so `devDependencies` are not available at runtime; when `npmCommand` is configured, git packages use plain `install` for compatibility with wrappers.
...
export default function (fuzzy: ExtensionAPI) {
  // Subscribe to events
  fuzzy.on("event_name", async (event, ctx) => {
  ...
  fuzzy.registerShortcut("ctrl+x", { ... });
  fuzzy.registerFlag("my-flag", { ... });
}
```

Extensions are loaded via [jiti](https://github.com/unjs/jiti), so TypeScript works without compilation.

If the factory returns a `Promise`, fuzzy awaits it before continuing startup. That means async initialization completes before `session_start`, before `resources_discover`, and before provider registrations queued via `fuzzy.registerProvider()` are flushed.
...
export default async function (fuzzy: ExtensionAPI) {
  ...
  fuzzy.registerProvider("local-openai", {
...
This pattern makes the fetched models available during normal startup and to `fuzzy --list-models`.
...
thinking level changes (settings, keybinding, fuzzy.setThinkingLevel())
...
```typescript
fuzzy.on("resources_discover", async (event, _ctx) => {
...
```typescript
fuzzy.on("session_before_switch", async (event, ctx) => {
...
After a successful switch or new-session action, fuzzy emits `session_shutdown` for the old extension instance, reloads and rebinds extensions for the new session, then emits `session_start` with `reason: "new" | "resume"` and `previousSessionFile`.
...
After a successful fork or clone, fuzzy emits `session_shutdown` for the old extension instance, reloads and rebinds extensions for the new session, then emits `session_start` with `reason: "fork"` and `previousSessionFile`.
Do cleanup work in `session_shutdown`, then reestablish any in-memory state in `session_start`.
...
```typescript
fuzzy.on("session_before_compact", async (event, ctx) => {
...
fuzzy.on("session_compact", async (event, ctx) => {
...
```typescript
fuzzy.on("session_before_tree", async (event, ctx) => {
...
fuzzy.on("session_tree", async (event, ctx) => {
...
```typescript
fuzzy.on("session_shutdown", async (event, ctx) => {
...
```typescript
fuzzy.on("before_agent_start", async (event, ctx) => {
...
The `systemPromptOptions` field gives extensions access to the same structured data Fuzzy uses to build the system prompt. This lets you inspect what Fuzzy has loaded — custom prompts, guidelines, tool snippets, context files, skills — without re-discovering resources or re-parsing flags. Use it when your extension needs to make deep, informed changes to the system prompt while respecting user-provided configuration.
...
fuzzy.on("agent_end", async (event, ctx) => {
...
```typescript
fuzzy.on("turn_start", async (event, ctx) => {
  // event.turnIndex, event.timestamp
});

fuzzy.on("turn_end", async (event, ctx) => {
...
```typescript
fuzzy.on("message_start", async (event, ctx) => {
  // event.message
});

fuzzy.on("message_update", async (event, ctx) => {
  // event.message
  // event.assistantMessageEvent (token-by-token stream event)
});

fuzzy.on("message_end", async (event, ctx) => {
...
fuzzy.on("tool_execution_update", async (event, ctx) => {
  // event.toolCallId, event.toolName, event.args, event.partialResult
});

fuzzy.on("tool_execution_end", async (event, ctx) => {
...
```typescript
fuzzy.on("context", async (event, ctx) => {
...
This hook can rewrite provider-level system instructions or remove them entirely. Those payload-level changes are not reflected by `ctx.getSystemPrompt()`, which reports Fuzzy's system prompt string rather than the final serialized provider payload.
...
```typescript
fuzzy.on("after_provider_response", (event, ctx) => {
...
```typescript
fuzzy.on("model_select", async (event, ctx) => {
...
```typescript
fuzzy.on("thinking_level_select", async (event, ctx) => {
...
Use this to update extension UI when `fuzzy.setThinkingLevel()`, model changes, or built-in thinking-level controls change the active thinking level.
...
```typescript
import { isBashToolResult } from "@fuzzyos/fuzzy-code";

fuzzy.on("tool_result", async (event, ctx) => {
...
```typescript
import { createLocalBashOperations } from "@fuzzyos/fuzzy-code";

fuzzy.on("user_bash", (event, ctx) => {
  ...
  // Option 2: Wrap fuzzy's built-in local bash backend
...
```typescript
fuzzy.on("input", async (event, ctx) => {
...
It is usually `undefined` in idle or non-turn contexts such as session events, extension commands, and shortcuts fired while fuzzy is idle.
...
Request a graceful shutdown of fuzzy.
...
Returns Fuzzy's current system prompt string.
...
```typescript
fuzzy.on("before_agent_start", (event, ctx) => {
...
Returns the base inputs Fuzzy currently uses to build the system prompt.
...
- `withSession`: run post-switch work against a fresh replacement-session context. Do not use captured old `fuzzy` / command `ctx`; see [Session replacement lifecycle and footguns](#session-replacement-lifecycle-and-footguns).
...
```typescript
import { SessionManager } from "@fuzzyos/fuzzy-code";

fuzzy.registerCommand("switch", {
...
- Captured old `fuzzy` / old command `ctx` session-bound objects are stale after replacement and will throw if used. Use only the `ctx` passed to `withSession` for session-bound work.
...
```typescript
fuzzy.registerCommand("reload-runtime", {
...
export default function (fuzzy: ExtensionAPI) {
  ...
  fuzzy.registerTool({
    ...
    parameters: Type.Object({}),
    async execute() {
      fuzzy.sendUserMessage("/reload-runtime", { deliverAs: "followUp" });
      return {
...
### fuzzy.on(event, handler)

Subscribe to events. See [Events](#events) for event types and return values.

### fuzzy.registerTool(definition)

Register a custom tool callable by the LLM. See [Custom Tools](#custom-tools) for full details.

`fuzzy.registerTool()` works both during extension load and after startup. You can call it inside `session_start`, command handlers, or other event handlers. New tools are refreshed immediately in the same session, so they appear in `fuzzy.getAllTools()` and are callable by the LLM without `/reload`.

Use `fuzzy.setActiveTools()` to enable or disable tools (including dynamically added tools) at runtime.
...
### fuzzy.sendMessage(message, options?)
...
### fuzzy.sendUserMessage(content, options?)

Send a user message to the agent. Unlike `sendMessage()` which sends custom messages, this sends an actual user message that appears as if typed by the user. Always triggers a turn.

```typescript
// Simple text message
fuzzy.sendUserMessage("What is 2+2?");
...
fuzzy.sendUserMessage("Focus on error handling", { deliverAs: "steer" });
fuzzy.sendUserMessage("And then summarize", { deliverAs: "followUp" });
```
...
### fuzzy.appendEntry(customType, data?)

Persist extension state (does NOT participate in LLM context).

```typescript
fuzzy.appendEntry("my-state", { count: 42 });
...
### fuzzy.setSessionName(name)

Set the session display name (shown in session selector instead of first message).

```typescript
fuzzy.setSessionName("Refactor auth module");
```

### fuzzy.getSessionName()

Get the current session name, if set.

```typescript
const name = fuzzy.getSessionName();
if (name) {
  console.log(`Session: ${name}`);
}
```

### fuzzy.setLabel(entryId, label)

Set or clear a label on an entry. Labels are user-defined markers for bookmarking and navigation (shown in `/tree` selector).

```typescript
// Set a label
fuzzy.setLabel(entryId, "checkpoint-before-refactor");

// Clear a label
fuzzy.setLabel(entryId, undefined);
...
### fuzzy.registerCommand(name, options)

Register a command.

If multiple extensions register the same command name, fuzzy keeps them all and assigns numeric invocation suffixes in load order, for example `/review:1` and `/review:2`.
...
```typescript
import type { AutocompleteItem } from "@fuzzyos/fuzzy-tui";
...
```typescript
const commands = fuzzy.getCommands();
...
### fuzzy.registerMessageRenderer(customType, renderer)

Register a custom TUI renderer for messages with your `customType`. See [Custom UI](#custom-ui).

### fuzzy.registerShortcut(shortcut, options)

Register a keyboard shortcut. See [keybindings.md](keybindings.md) for the shortcut format and built-in keybindings.

```typescript
fuzzy.registerShortcut("ctrl+shift+p", {
...
### fuzzy.registerFlag(name, options)

Register a CLI flag.

```typescript
fuzzy.registerFlag("plan", {
...
// Check value
if (fuzzy.getFlag("plan")) {
  // Plan mode enabled
}
```

### fuzzy.exec(command, args, options?)

Execute a shell command.

```typescript
const result = await fuzzy.exec("git", ["status"], { signal, timeout: 5000 });
// result.stdout, result.stderr, result.code, result.killed
```

### fuzzy.getActiveTools() / fuzzy.getAllTools() / fuzzy.setActiveTools(names)

Manage active tools. This works for both built-in tools and dynamically registered tools.

```typescript
const active = fuzzy.getActiveTools();
const all = fuzzy.getAllTools();
// [{
//   name: "read",
...
fuzzy.setActiveTools(["read", "bash"]); // Switch to read-only
```

`fuzzy.getAllTools()` returns `name`, `description`, `parameters`, `promptGuidelines`, and `sourceInfo`.
...
if (model) {
  const success = await fuzzy.setModel(model);
  if (!success) {
...
### fuzzy.getThinkingLevel() / fuzzy.setThinkingLevel(level)
...
```typescript
const current = fuzzy.getThinkingLevel();  // "off" | "minimal" | "low" | "medium" | "high" | "xhigh"
fuzzy.setThinkingLevel("high");
```
...
```typescript
fuzzy.events.on("my:event", (data) => { ... });
fuzzy.events.emit("my:event", { ... });
```

### fuzzy.registerProvider(name, config)

Register or override a model provider dynamically. Useful for proxies, custom endpoints, or team-wide model configurations.

Calls made during the extension factory function are queued and applied once the runner initialises. Calls made after that — for example from a command handler following a user setup flow — take effect immediately without requiring a `/reload`.

If you need to discover models from a remote endpoint, prefer an async extension factory over deferring the fetch to `session_start`. fuzzy waits for the factory before startup continues, so the registered models are available immediately, including to `fuzzy --list-models`.

```typescript
// Register a new provider with custom models
fuzzy.registerProvider("my-proxy", {
  name: "My Proxy",
...
fuzzy.registerProvider("anthropic", {
  baseUrl: "https://proxy.example.com"
});

// Register provider with OAuth support for /login
fuzzy.registerProvider("corporate-ai", {
...
```typescript
fuzzy.registerCommand("my-setup-teardown", {
  description: "Remove the custom proxy provider",
  handler: async (_args, _ctx) => {
    fuzzy.unregisterProvider("my-proxy");
  },
});
```
...
Register tools the LLM can call via `fuzzy.registerTool()`. Tools appear in the system prompt and can have custom rendering.

Use `promptSnippet` for a short one-line entry in the `Available tools` section in the default system prompt. If omitted, custom tools are left out of that section.

Use `promptGuidelines` to add tool-specific bullets to the default system prompt `Guidelines` section. These bullets are included only while the tool is active (for example, after `fuzzy.setActiveTools([...])`).
...
```typescript
import { withFileMutationQueue } from "@fuzzyos/fuzzy-code";
...
import { Type } from "typebox";
import { StringEnum } from "@fuzzyos/fuzzy-ai";
import { Text } from "@fuzzyos/fuzzy-tui";
...
fuzzy.registerTool({
  ...
  async execute(toolCallId, params, signal, onUpdate, ctx) {
    ...
    // Run commands via fuzzy.exec (captured from extension closure)
    const result = await fuzzy.exec("some-command", [], { signal });
...
**Argument preparation:** `prepareArguments(args)` is optional. If defined, it runs before schema validation and before `execute()`. Use it to mimic an older accepted input shape when fuzzy resumes an older session whose stored tool call arguments no longer match the current schema. Return the object you want validated against `parameters`. Keep the public schema strict. Do not add deprecated compatibility fields to `parameters` just to keep old resumed sessions working.
...
fuzzy -e ./tool-override.ts
```

Alternatively, use `--no-builtin-tools` to start without any built-in tools while keeping extension tools enabled:
```bash
# No built-in tools, only extension tools
fuzzy --no-builtin-tools -e ./my-extension.ts
```
...
- [read.ts](https://github.com/fuzzyos/fuzzyos/blob/main/packages/fuzzy-code/src/core/tools/read.ts) - `ReadToolDetails`
- [bash.ts](https://github.com/fuzzyos/fuzzyos/blob/main/packages/fuzzy-code/src/core/tools/bash.ts) - `BashToolDetails`
- [edit.ts](https://github.com/fuzzyos/fuzzyos/blob/main/packages/fuzzy-code/src/core/tools/edit.ts)
- [write.ts](https://github.com/fuzzyos/fuzzyos/blob/main/packages/fuzzy-code/src/core/tools/write.ts)
- [grep.ts](https://github.com/fuzzyos/fuzzyos/blob/main/packages/fuzzy-code/src/core/tools/grep.ts) - `GrepToolDetails`
- [find.ts](https://github.com/fuzzyos/fuzzyos/blob/main/packages/fuzzy-code/src/core/tools/find.ts) - `FindToolDetails`
- [ls.ts](https://github.com/fuzzyos/fuzzyos/blob/main/packages/fuzzy-code/src/core/tools/ls.ts) - `LsToolDetails`
...
```typescript
import { createReadTool, createBashTool, type ReadOperations } from "@fuzzyos/fuzzy-code";
...
For `user_bash`, extensions can reuse fuzzy's local shell backend via `createLocalBashOperations()` instead of reimplementing local process spawning, shell resolution, and process-tree termination.
...
export default function (fuzzy: ExtensionAPI) {
  ...
  fuzzy.registerTool({ name: "db_connect", ... });
  fuzzy.registerTool({ name: "db_query", ... });
  fuzzy.registerTool({ name: "db_close", ... });

  fuzzy.on("session_shutdown", async () => {
    connection?.close();
  });
}
```
...
Tools can provide `renderCall` and `renderResult` for custom TUI display. See [tui.md](tui.md) for the full component API and [tool-execution.ts](https://github.com/fuzzyos/fuzzyos/blob/main/packages/fuzzy-code/src/modes/interactive/components/tool-execution.ts) for how tool rows are composed.
...
```typescript
import { keyHint } from "@fuzzyos/fuzzy-code";
...
// Terminal title
ctx.ui.setTitle("fuzzy - my-project");
...
```typescript
import { Text, Component } from "@fuzzyos/fuzzy-tui";
...
```typescript
import { CustomEditor, type ExtensionAPI } from "@fuzzyos/fuzzy-code";
import { matchesKey } from "@fuzzyos/fuzzy-tui";
...
fuzzy.registerMessageRenderer("my-extension", (message, options, theme) => {
...
Messages are sent via `fuzzy.sendMessage()`:
...
```typescript
import { highlightCode, getLanguageFromPath } from "@fuzzyos/fuzzy-code";
...
| `event-bus.ts` | Inter-extension events | `fuzzy.events` |
| **Session Metadata** |||
