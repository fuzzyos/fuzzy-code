...
The `edit` tool returns `details.diff` for Fuzzy's TUI display and `details.patch` as a standard unified patch for SDK consumers.
...
import { createAgentSession, defineTool } from "@fuzzyos/fuzzy-code";
...
Use `defineTool()` for standalone definitions and arrays like `customTools: [myTool]`. Inline `fuzzy.registerTool({ ... })` already infers parameter types correctly.
