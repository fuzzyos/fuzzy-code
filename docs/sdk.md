> fuzzy can help you use the SDK. Ask it to build an integration for your use case.

# SDK

The SDK provides programmatic access to fuzzy's agent capabilities. Use it to embed fuzzy in other applications, build custom interfaces, or integrate with automated workflows.
...
```bash
npm install @fuzzyos/fuzzy-code
```
...
```typescript
import { createAgentSession } from "@fuzzyos/fuzzy-code";
...
**Behavior:**
- **Extension commands** (e.g., `/mycommand`): Execute immediately, even during streaming. They manage their own LLM interaction via `fuzzy.sendMessage()`.
...
The `Agent` class (from `@fuzzyos/fuzzy-agent`) handles the core LLM interaction. Access it via `session.agent`.
...
const { session } = await createAgentSession({
  ...
  agentDir: "~/.fuzzy/agent", // default (expands ~)
});
```

`cwd` is used by `DefaultResourceLoader` for:
- Project extensions (`.fuzzy/extensions/`)
- Project skills:
  - `.fuzzy/skills/`
  - `.agents/skills/` in `cwd` and ancestor directories (up to git repo root, or filesystem root when not in a repo)
- Project prompts (`.fuzzy/prompts/`)
...
- Global skills:
  - `skills/` under `agentDir` (for example `~/.fuzzy/agent/skills/`)
  - `~/.agents/skills/`
...
import { AuthStorage, ModelRegistry } from "@fuzzyos/fuzzy-code";
...
// Default: uses ~/.fuzzy/agent/auth.json and ~/.fuzzy/agent/models.json
const authStorage = AuthStorage.create();
const modelRegistry = new ModelRegistry(authStorage);
...
```typescript
import { createAgentSession, DefaultResourceLoader } from "@fuzzyos/fuzzy-code";
...
- If you omit `tools`, fuzzy automatically creates them with the correct `cwd`
...
import { createAgentSession, type ToolDefinition } from "@fuzzyos/fuzzy-code";
...
Custom tools passed via `customTools` are combined with extension-registered tools. Extensions loaded by the ResourceLoader can also register tools via `fuzzy.registerTool()`.

> See [examples/sdk/05-tools.ts](../examples/sdk/05-tools.ts)

### Extensions

Extensions are loaded by the `ResourceLoader`. `DefaultResourceLoader` discovers extensions from `~/.fuzzy/agent/extensions/`, `.fuzzy/extensions/`, and settings.json extension sources.
...
**Event Bus:** Extensions can communicate via `fuzzy.events`. Pass a shared `eventBus` to `DefaultResourceLoader` if you need to emit or listen from outside:

```typescript
import { createEventBus, DefaultResourceLoader } from "@fuzzyos/fuzzy-code";
...
```typescript
import { createAgentSession, SessionManager } from "@fuzzyos/fuzzy-code";
...
```typescript
import { createAgentSession, SettingsManager, SessionManager } from "@fuzzyos/fuzzy-code";
...
1. Global: `~/.fuzzy/agent/settings.json`
2. Project: `<cwd>/.fuzzy/settings.json`
...
```typescript
import { createAgentSession, InteractiveMode } from "@fuzzyos/fuzzy-code";
...
```typescript
import { createAgentSession, runPrintMode } from "@fuzzyos/fuzzy-code";
...
```typescript
import { createAgentSession, runRpcMode } from "@fuzzyos/fuzzy-code";
