import { Markdown, type MarkdownTheme } from "@fuzzyos/fuzzy-tui";
...
function getPackageCommandUsage(command: PackageCommand): string {
	switch (command) {
		...
		case "update":
			return `${APP_NAME} update [source|self|fuzzy] [--self] [--extensions] [--extension <source>] [--force]`;
		case "list":
...
Options:
  -l, --local    Install project-locally (.fuzzy/settings.json)
...
Update fuzzy and installed packages.

Options:
  --self                  Update fuzzy only
  --extensions            Update installed packages only
  --extension <source>    Update one package only
  --force                 Reinstall fuzzy even if the current version is latest

Short forms:
  ${APP_NAME} update                Update fuzzy and all extensions
  ${APP_NAME} update <source>       Update one package
  ${APP_NAME} update fuzzy             Update fuzzy only (self works as alias to fuzzy)
`);
			return;
...
function parsePackageCommand(args: string[]): PackageCommandOptions | undefined {
	...
	if (command === "update") {
		if (extensionFlagSource) {
			...
			const sourceIsSelf = source === "self" || source === "fuzzy";
			if (sourceIsSelf) {
...
function printSelfUpdateUnavailable(npmCommand?: string[], updatePackageName = PACKAGE_NAME): void {
	...
	if (entrypoint) {
		console.error("");
		console.error(`Location of fuzzy executable: ${entrypoint}`);
	}
}
