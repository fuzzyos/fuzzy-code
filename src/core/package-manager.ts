...
function readPiManifestFile(packageJsonPath: string): PiManifest | null {
	try {
		...
		const pkg = JSON.parse(content) as { fuzzy?: PiManifest };
...
export class DefaultPackageManager implements PackageManager {
	...
	private getNpmInstallArgs(specs: string[], installRoot: string): string[] {
		...
		// Extension packages run inside fuzzy and resolve fuzzy APIs through loader aliases/virtual modules.
		// Disable peer dependency resolution for managed installs (npm's --legacy-peer-deps, and
		// equivalent bun/pnpm settings) so package managers do not install or solve host-provided
		// @fuzzyos/fuzzy-* peers. Stale auto-installed fuzzy peers can otherwise block updates.
	...
	private addAutoDiscoveredResources(
		...
		// User extensions from ~/.fuzzy/agent/
		addResources(
			"extensions",
