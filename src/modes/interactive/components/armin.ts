...
import type { Component, TUI } from "@fuzzyos/fuzzy-tui";
...
function getFuzzyxel(x: number, y: number): boolean {
...
function getChar(x: number, row: number): string {
	const upper = getFuzzyxel(x, row * 2);
	const lower = getFuzzyxel(x, row * 2 + 1);
