...
Content`,
	...
	describe("source parsing", () => {
		...
		it("should never parse dot-relative paths as git", () => {
			const dotSlash = (packageManager as any).parseSource("./packages/fuzzy-agent-timers");
			expect(dotSlash.type).toBe("local");
			expect(dotSlash.path).toBe("./packages/fuzzy-agent-timers");

			const dotDotSlash = (packageManager as any).parseSource("../packages/fuzzy-agent-timers");
			expect(dotDotSlash.type).toBe("local");
			expect(dotDotSlash.path).toBe("../packages/fuzzy-agent-timers");
		});
	});
...
export default function(api) { api.registerTool({ name: "test", description: "test", execute: async () => helper() }); }`,
	...
	describe("offline mode and network timeouts", () => {
		it("should update project npm packages using @latest", async () => {
			...
			expect(runCommandSpy).toHaveBeenCalledWith(
				"npm",
				["install", "example@latest", "--prefix", join(tempDir, ".fuzzy", "npm")],
				undefined,
			);
		});
