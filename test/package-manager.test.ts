...
describe("DefaultPackageManager", () => {
	...
	beforeEach(() => {
		previousOfflineEnv = process.env.FUZZY_OFFLINE;
		delete process.env.FUZZY_OFFLINE;
	...
	afterEach(() => {
		if (previousOfflineEnv === undefined) {
			...
			process.env.FUZZY_OFFLINE = previousOfflineEnv;
		}
		vi.restoreAllMocks();
		vi.unstubAllGlobals();
...
Content`,
		...
		it("should auto-discover root markdown skills from .fuzzy skill dirs", async () => {
...
Content`,
		...
		it("should resolve project paths relative to .fuzzy", async () => {
			const extDir = join(tempDir, ".fuzzy", "extensions");
			mkdirSync(extDir, { recursive: true });
...
Content`,
				...
				mkdirSync(join(tempDir, ".fuzzy"), { recursive: true });
				...
				symlinkSync(sharedExtensionsDir, join(tempDir, ".fuzzy", "extensions"), "dir");
				symlinkSync(sharedSkillsDir, join(tempDir, ".fuzzy", "skills"), "dir");
				symlinkSync(sharedPromptsDir, join(tempDir, ".fuzzy", "prompts"), "dir");
				symlinkSync(sharedThemesDir, join(tempDir, ".fuzzy", "themes"), "dir");
		...
		it("should auto-discover project prompts with overrides", async () => {
			const promptsDir = join(tempDir, ".fuzzy", "prompts");
			mkdirSync(promptsDir, { recursive: true });
		...
		it("should resolve directory with package.json fuzzy.extensions in extensions setting", async () => {
			// Create a package with fuzzy.extensions in package.json
			...
			// Should find the extensions declared in package.json fuzzy.extensions
	...
	describe("auto-discovered skill metadata", () => {
		it("should use the agent dir as baseDir for user .fuzzy/agent skills", async () => {
			const skillPath = join(agentDir, "skills", "user-fuzzy", "SKILL.md");
			mkdirSync(join(agentDir, "skills", "user-fuzzy"), { recursive: true });
			writeFileSync(skillPath, "---\nname: user-fuzzy\ndescription: user fuzzy\n---\n");
		...
		it("should use the project .fuzzy dir as baseDir for project .fuzzy skills", async () => {
			const projectBaseDir = join(tempDir, ".fuzzy");
			const skillPath = join(projectBaseDir, "skills", "project-fuzzy", "SKILL.md");
			mkdirSync(join(projectBaseDir, "skills", "project-fuzzy"), { recursive: true });
			writeFileSync(skillPath, "---\nname: project-fuzzy\ndescription: project fuzzy\n---\n");
	...
	describe(".agents/skills auto-discovery", () => {
		...
		it("should keep ~/.agents/skills user-scoped when cwd is under home in a non-git directory", async () => {
			...
			try {
				...
				const localAgentDir = join(tempDir, ".fuzzy", "agent");
		...
		it("should dedupe user skill entries when ~/.fuzzy/agent/skills is a symlink to ~/.agents/skills", async () => {
			const previousHome = process.env.HOME;
			process.env.HOME = tempDir;
	...
	describe("ignore files", () => {
		...
		it("should not apply parent .gitignore to .fuzzy auto-discovery", async () => {
			writeFileSync(join(tempDir, ".gitignore"), ".fuzzy\n");

			const skillDir = join(tempDir, ".fuzzy", "skills", "auto-skill");
			mkdirSync(skillDir, { recursive: true });
	...
	describe("resolveExtensionSources", () => {
		...
		it("should handle directories with fuzzy manifest", async () => {
		...
		it("should keep fuzzy manifest entries with leading tilde package-relative", async () => {
	...
	describe("command spawning", () => {
		it("should preserve argv entries containing spaces", () => {
			...
			const valueWithSpace = "C:\\Users\\A B\\.fuzzy\\npm";
	...
	describe("npmCommand", () => {
		...
		it("should update git package dependencies with --omit=dev", async () => {
			const source = "git:github.com/user/repo";
			const targetDir = join(tempDir, ".fuzzy", "git", "github.com", "user", "repo");
			mkdirSync(targetDir, { recursive: true });
			writeFileSync(join(targetDir, "package.json"), JSON.stringify({ name: "repo", version: "1.0.0" }));
		...
		it("should install user npm packages into the fuzzy-managed npm root", async () => {
			settingsManager = SettingsManager.inMemory({
				npmCommand: ["pnpm"],
	...
	describe("temporary install paths", () => {
		it("should place temporary npm packages under the agent temp extension folder", () => {
			...
			expect(installPath.startsWith(join(tmpdir(), "fuzzy-extensions"))).toBe(false);
	...
	describe("settings source normalization", () => {
		...
		it("should store project local packages relative to .fuzzy settings base", () => {
			...
			const rel = relative(join(tempDir, ".fuzzy"), projectPkgDir);
			const expected = rel.startsWith(".") ? rel : `./${rel}`;
			expect(settings.packages?.[0]).toBe(expected);
		});
	...
	describe("pattern filtering in fuzzy manifest", () => {
...
export default function(api) { api.registerTool({ name: "test", description: "test", execute: async () => helper() }); }`,
		...
		it("should respect package.json fuzzy.extensions manifest in subdirectories", async () => {
	...
	describe("offline mode and network timeouts", () => {
		it("should update project npm packages using @latest when newer version is available", async () => {
			const installedPath = join(tempDir, ".fuzzy", "npm", "node_modules", "example");
			...
			expect(runCommandSpy).toHaveBeenCalledWith(
				"npm",
				["install", "example@latest", "--prefix", join(tempDir, ".fuzzy", "npm"), "--legacy-peer-deps"],
				undefined,
			);
		});
		...
		it("should batch npm updates per scope and run git updates in parallel while skipping pinned npm and current packages", async () => {
			...
			const projectOldPath = join(tempDir, ".fuzzy", "npm", "node_modules", "project-old");
			const projectCurrentPath = join(tempDir, ".fuzzy", "npm", "node_modules", "project-current");
		...
		it("should skip pinned packages when checking for updates", async () => {
			const installedNpmPath = join(tempDir, ".fuzzy", "npm", "node_modules", "example");
