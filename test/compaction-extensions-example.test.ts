...
describe("Documentation example", () => {
	it("custom compaction example should type-check correctly", () => {
		...
		const exampleExtension = (fuzzy: ExtensionAPI) => {
			fuzzy.on("session_before_compact", async (event: SessionBeforeCompactEvent, ctx) => {
	...
	it("compact event should have correct fields", () => {
		const checkCompactEvent = (fuzzy: ExtensionAPI) => {
			fuzzy.on("session_compact", async (event: SessionCompactEvent) => {
