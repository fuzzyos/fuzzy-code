...
import type { AnthropicMessagesCompat, Api, Context, Model, OpenAICompletionsCompat } from "@fuzzyos/fuzzy-ai";
import { getApiProvider } from "@fuzzyos/fuzzy-ai";
import { getOAuthProvider } from "@fuzzyos/fuzzy-ai/oauth";
...
describe("ModelRegistry", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `fuzzy-test-model-registry-${Date.now()}-${Math.random().toString(36).slice(2)}`);
