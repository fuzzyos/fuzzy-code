...
import { Agent, type ThinkingLevel } from "@fuzzyos/fuzzy-agent";
import { getModel, type Model } from "@fuzzyos/fuzzy-ai";
...
describe.skipIf(!HAS_ANTIGRAVITY_AUTH)("Compaction with thinking models (Antigravity)", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `fuzzy-thinking-compaction-test-${Date.now()}`);
...
describe.skipIf(!HAS_ANTHROPIC_AUTH)("Compaction with thinking models (Anthropic)", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `fuzzy-thinking-compaction-anthropic-test-${Date.now()}`);
		mkdirSync(tempDir, { recursive: true });
	});
