/**
 * Local test harness for the new fuzzy-code test suite.
 */

import { existsSync, mkdirSync, rmSync } from "node:fs";
import { tmpdir } from "node:os";
import { join } from "node:path";
import type { AgentMessage, AgentTool } from "@fuzzyos/fuzzy-agent";
import { Agent } from "@fuzzyos/fuzzy-agent";
import type { FauxModelDefinition, FauxProviderRegistration, FauxResponseStep, Model } from "@fuzzyos/fuzzy-ai";
import { registerFauxProvider } from "@fuzzyos/fuzzy-ai";
...
function createTempDir(): string {
	const tempDir = join(tmpdir(), `fuzzy-suite-${Date.now()}-${Math.random().toString(36).slice(2)}`);
