...
/**
 * Regression test for https://github.com/fuzzyos/fuzzyos/issues/2791
 *
...
describe("issue #2791 fs.watch error event crashes process", () => {
	...
	beforeEach(() => {
		tempRoot = mkdtempSync(join(tmpdir(), "fuzzy-2791-"));
...
process.env.FUZZY_CODE_DIR = "${agentDir}";
...
`,
		...
		try {
			_stdout = execFileSync("npx", ["tsx", scriptPath], {
				timeout: 10000,
				encoding: "utf-8",
				env: { ...process.env, FUZZY_CODE_DIR: agentDir },
