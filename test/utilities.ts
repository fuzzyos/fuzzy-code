/**
 * Shared test utilities for fuzzy-code tests.
 */
...
import { getModel, type OAuthCredentials, type OAuthProvider } from "@fuzzyos/fuzzy-ai";
import { getOAuthApiKey } from "@fuzzyos/fuzzy-ai/oauth";
...
// ============================================================================
// OAuth API key resolution from ~/.fuzzy/agent/auth.json
// ============================================================================

const AUTH_PATH = join(homedir(), ".fuzzy", "agent", "auth.json");
...
/**
 * Resolve API key for a provider from ~/.fuzzy/agent/auth.json
 *
...
/**
 * Check if a provider has credentials in ~/.fuzzy/agent/auth.json
 */
...
/** Path to the real fuzzy agent config directory */
export const FUZZY_AGENT_DIR = join(homedir(), ".fuzzy", "agent");

/**
 * Get an AuthStorage instance backed by ~/.fuzzy/agent/auth.json
...
export function createTestSession(options: TestSessionOptions = {}): TestSessionContext {
	const tempDir = join(tmpdir(), `fuzzy-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
