...
import { Agent, type AgentEvent, type AgentTool } from "@fuzzyos/fuzzy-agent";
import { type AssistantMessage, type AssistantMessageEvent, EventStream, getModel } from "@fuzzyos/fuzzy-ai";
...
describe("AgentSession retry", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `fuzzy-retry-test-${Date.now()}`);
