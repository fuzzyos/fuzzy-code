...
describe("sessionDir precedence", () => {
	...
	beforeEach(() => {
		...
		tempDir = join(tmpdir(), `fuzzy-session-dir-${Date.now()}-${Math.random().toString(36).slice(2)}`);
	...
	it("prefers settings sessionDir over the session_directory hook for new sessions", async () => {
		writeFileSync(join(projectDir, ".fuzzy", "settings.json"), JSON.stringify({ sessionDir: "./settings-sessions" }));
