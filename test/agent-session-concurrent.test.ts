...
describe("AgentSession concurrent prompt guard", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `fuzzy-concurrent-test-${Date.now()}`);
	...
	it("should queue extension-origin steering messages while streaming", async () => {
		...
		const extensionsResult = await createTestExtensionsResult([
			(fuzzy) => {
				(globalThis as typeof globalThis & { testExtensionApi?: unknown }).testExtensionApi = fuzzy;
			},
			(fuzzy) => {
		...
		expect(_fuzzy).toBeDefined();

		_fuzzy!.sendUserMessage("Steer from extension", { deliverAs: "steer" });
