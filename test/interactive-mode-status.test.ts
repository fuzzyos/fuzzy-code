...
import { type AutocompleteProvider, CombinedAutocompleteProvider } from "@fuzzyos/fuzzy-tui";
import { beforeAll, describe, expect, test, vi } from "vitest";
import { type Component, Container, type Focusable, TUI } from "../../fuzzy-tui/src/tui.ts";
import { VirtualTerminal } from "../../fuzzy-tui/test/virtual-terminal.ts";
...
describe("InteractiveMode.showLoadedResources", () => {
	...
	function createExtensionFixtures(): ExtensionFixture[] {
		return [
			{
				path: "/tmp/project/.fuzzy/extensions/answer.ts",
				sourceInfo: createSourceInfo("/tmp/project/.fuzzy/extensions/answer.ts", {
					source: "local",
					scope: "project",
					origin: "top-level",
					baseDir: "/tmp/project/.fuzzy/extensions",
				}),
			},
			{
				path: "/tmp/project/.fuzzy/extensions/local-index/index.ts",
				sourceInfo: createSourceInfo("/tmp/project/.fuzzy/extensions/local-index/index.ts", {
					source: "local",
					scope: "project",
					origin: "top-level",
					baseDir: "/tmp/project/.fuzzy/extensions",
			...
			{
				path: "/tmp/project/.fuzzy/npm/node_modules/fuzzy-markdown-preview/extensions/index.ts",
				sourceInfo: createSourceInfo(
					"/tmp/project/.fuzzy/npm/node_modules/fuzzy-markdown-preview/extensions/index.ts",
					{
						source: "npm:fuzzy-markdown-preview",
						scope: "project",
						origin: "package",
						baseDir: "/tmp/project/.fuzzy/npm/node_modules/fuzzy-markdown-preview",
					},
				),
			},
			{
				path: "/tmp/project/.fuzzy/npm/node_modules/@scope/fuzzy-scoped/extensions/index.ts",
				sourceInfo: createSourceInfo(
					"/tmp/project/.fuzzy/npm/node_modules/@scope/fuzzy-scoped/extensions/index.ts",
					{
						source: "npm:@scope/fuzzy-scoped",
						scope: "project",
						origin: "package",
						baseDir: "/tmp/project/.fuzzy/npm/node_modules/@scope/fuzzy-scoped",
					},
				),
			},
			{
				path: "/tmp/project/.fuzzy/git/github.com/HazAT/fuzzy-interactive-subagents/extensions/index.ts",
				sourceInfo: createSourceInfo(
					"/tmp/project/.fuzzy/git/github.com/HazAT/fuzzy-interactive-subagents/extensions/index.ts",
					{
						source: "git:github.com/HazAT/fuzzy-interactive-subagents",
						scope: "project",
						origin: "package",
						baseDir: "/tmp/project/.fuzzy/git/github.com/HazAT/fuzzy-interactive-subagents",
					},
				),
			},
			{
				path: "/tmp/project/.fuzzy/git/github.com/HazAT/fuzzy-interactive-subagents/extensions/subagents/index.ts",
				sourceInfo: createSourceInfo(
					"/tmp/project/.fuzzy/git/github.com/HazAT/fuzzy-interactive-subagents/extensions/subagents/index.ts",
					{
						source: "git:github.com/HazAT/fuzzy-interactive-subagents",
						scope: "project",
						origin: "package",
...
"[Extensions]
  @scope/fuzzy-scoped, answer.ts, cli-extension.ts, fuzzy-markdown-preview, HazAT/fuzzy-interactive-subagents, HazAT/fuzzy-interactive-subagents:subagents, local-index, user-index"`);
	});
...
"[Extensions]
  project
    /tmp/project/.fuzzy/extensions/answer.ts
    /tmp/project/.fuzzy/extensions/local-index
    git:github.com/HazAT/fuzzy-interactive-subagents
      extensions
      extensions/subagents
    npm:@scope/fuzzy-scoped
      extensions
    npm:fuzzy-markdown-preview
      extensions
  user
  ...
  path
	...
	test("shows context paths relative to cwd while preserving full external paths", () => {
		const home = homedir();
		const cwd = path.join(home, "Development", "fuzzyos");
		const fakeThis = createShowLoadedResourcesThis({
			quietStartup: false,
			cwd,
			contextFiles: [
				{ path: path.join(home, ".fuzzy", "agent", "AGENTS.md") },
		...
		expect(output).toContain("~/.fuzzy/agent/AGENTS.md, AGENTS.md");
	...
	test("shows full context paths when expanded", () => {
		...
		const output = renderAll(fakeThis.chatContainer).replace(/\\/g, "/");
		expect(output).toContain("[Context]");
		expect(output).toContain("~/.fuzzy/agent/AGENTS.md");
		expect(output).toContain("~/Development/fuzzyos/AGENTS.md");
		expect(output).not.toContain("~/.fuzzy/agent/AGENTS.md, AGENTS.md");
	});
