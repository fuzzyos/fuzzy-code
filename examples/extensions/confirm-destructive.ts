...
import type { ExtensionAPI, SessionBeforeSwitchEvent, SessionMessageEntry } from "@fuzzyos/fuzzy-code";

export default function (fuzzy: ExtensionAPI) {
	fuzzy.on("session_before_switch", async (event: SessionBeforeSwitchEvent, ctx) => {
		if (!ctx.hasUI) return;
