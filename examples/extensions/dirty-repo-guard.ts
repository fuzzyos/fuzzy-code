...
async function checkDirtyRepo(
	...
	const { stdout, code } = await fuzzy.exec("git", ["status", "--porcelain"]);
...
export default function (fuzzy: ExtensionAPI) {
	fuzzy.on("session_before_switch", async (event, ctx) => {
		...
		return checkDirtyRepo(fuzzy, ctx, action);
	});

	fuzzy.on("session_before_fork", async (_event, ctx) => {
		return checkDirtyRepo(fuzzy, ctx, "fork");
	});
}
