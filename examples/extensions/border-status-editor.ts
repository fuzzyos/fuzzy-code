import { CustomEditor, type ExtensionAPI, type ExtensionContext, type KeybindingsManager } from "@fuzzyos/fuzzy-code";
import type { Component, EditorTheme, TUI } from "@fuzzyos/fuzzy-tui";
import { truncateToWidth, visibleWidth } from "@fuzzyos/fuzzy-tui";
...
export default function (fuzzy: ExtensionAPI) {
	...
	fuzzy.on("session_start", (_event, ctx) => {
		...
		const refreshBranch = async () => {
			const result = await fuzzy.exec("git", ["branch", "--show-current"], { cwd: ctx.cwd }).catch(() => undefined);
		...
		class BorderStatusEditor extends CustomEditor {
			...
			render(width: number): string[] {
				...
				const thinking = fuzzy.getThinkingLevel();
