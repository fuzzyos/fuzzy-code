...
export default function (fuzzy: ExtensionAPI) {
	...
	fuzzy.on("turn_start", async (_event, ctx) => {
		turnCount++;
		ctx.ui.setStatus("rpc-demo", `Turn ${turnCount} running...`);
	});

	fuzzy.on("turn_end", async (_event, ctx) => {
	...
	fuzzy.registerCommand("rpc-input", {
	...
	fuzzy.registerCommand("rpc-editor", {
	...
	fuzzy.registerCommand("rpc-prefill", {
