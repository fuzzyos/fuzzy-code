 ...
 *
 * Usage:
 *   fuzzy -e ./ssh.ts --ssh user@host
 *   fuzzy -e ./ssh.ts --ssh user@host:/remote/path
 *
 * Requirements:
...
export default function (fuzzy: ExtensionAPI) {
	fuzzy.registerFlag("ssh", { description: "SSH remote: user@host or user@host:/path", type: "string" });
	...
	fuzzy.on("session_start", async (_event, ctx) => {
		...
		const arg = fuzzy.getFlag("ssh") as string | undefined;
		if (arg) {
			if (arg.includes(":")) {
