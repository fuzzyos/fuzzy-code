...
Example extensions for fuzzy-code.

## Usage

```bash
# Load an extension with --extension flag
fuzzy --extension examples/extensions/permission-gate.ts

# Or copy to extensions directory for auto-discovery
cp permission-gate.ts ~/.fuzzy/agent/extensions/
```
...
| `send-user-message.ts` | Demonstrates `fuzzy.sendUserMessage()` for sending user messages from extensions |
...
| Extension | Description |
|-----------|-------------|
| `mac-system-theme.ts` | Syncs fuzzy theme with macOS dark/light mode |
...
| `event-bus.ts` | Inter-extension communication via `fuzzy.events` |
...
| `custom-provider-gitlab-duo/` | GitLab Duo provider using fuzzy-ai's built-in Anthropic/OpenAI streaming via proxy |
