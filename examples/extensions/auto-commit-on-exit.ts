...
export default function (fuzzy: ExtensionAPI) {
	fuzzy.on("session_shutdown", async (_event, ctx) => {
		...
		const { stdout: status, code } = await fuzzy.exec("git", ["status", "--porcelain"]);
		...
		const commitMessage = `[fuzzy] ${firstLine.slice(0, 50)}${firstLine.length > 50 ? "..." : ""}`;

		// Stage and commit
		await fuzzy.exec("git", ["add", "-A"]);
		const { code: commitCode } = await fuzzy.exec("git", ["commit", "-m", commitMessage]);
