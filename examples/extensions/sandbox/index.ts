 ...
 *
 * Usage:
 * - `fuzzy -e ./sandbox` - sandbox enabled with default/config settings
 * - `fuzzy -e ./sandbox --no-sandbox` - disable sandboxing
 * - `/sandbox` - show current sandbox configuration
 *
 * Setup:
 * 1. Copy sandbox/ directory to ~/.fuzzy/agent/extensions/
 * 2. Run `npm install` in ~/.fuzzy/agent/extensions/sandbox/
 *
...
export default function (fuzzy: ExtensionAPI) {
	...
	fuzzy.on("session_start", async (_event, ctx) => {
		const noSandbox = fuzzy.getFlag("no-sandbox") as boolean;
