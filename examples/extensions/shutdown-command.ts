 ...
 * Demonstrates how extensions can use ctx.shutdown() to exit fuzzy cleanly.
 */
...
export default function (fuzzy: ExtensionAPI) {
	// Register a /quit command that cleanly exits fuzzy
	fuzzy.registerCommand("quit", {
		description: "Exit fuzzy cleanly",
	...
	fuzzy.registerTool({
		...
		description: "Complete a task and exit fuzzy",
	...
	fuzzy.registerTool({
		...
		description: "Deploy the application and exit fuzzy",
		parameters: Type.Object({
		...
		async execute(_toolCallId, params, _signal, onUpdate, ctx) {
			...
			// const result = await fuzzy.exec("npm", ["run", "deploy", params.environment], { signal });
