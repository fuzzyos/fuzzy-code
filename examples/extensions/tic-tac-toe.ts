...
import type { ExtensionAPI, ExtensionContext, Theme, ToolExecutionMode } from "@fuzzyos/fuzzy-code";
import { type Component, matchesKey, Text, truncateToWidth, visibleWidth } from "@fuzzyos/fuzzy-tui";
...
export default function (fuzzy: ExtensionAPI) {
	...
	// -----------------------------------------------------------------------
	fuzzy.registerMessageRenderer(MOVE_MESSAGE_TYPE, (message, { expanded }, theme) => {
	...
	// -----------------------------------------------------------------------
	fuzzy.registerMessageRenderer(GAME_OVER_MESSAGE_TYPE, (message, _options, theme) => {
		const details = message.details as BoardDetails | undefined;
...
`;
	...
	// /tic-tac-toe command
	// -----------------------------------------------------------------------
	fuzzy.registerCommand("tic-tac-toe", {
		...
		handler: async (_args, ctx) => {
			...
			gameActive = true;
			fuzzy.setSessionName("Tic-Tac-Toe");
			...
			await ctx.ui.custom<void>((tui, _theme, _kb, done) => {
				component = new TicTacToeComponent(
					...
					(row, col) => {
						...
						fuzzy.appendEntry(SAVE_TYPE, getBoardDetails());
