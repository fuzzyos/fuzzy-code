...
async function resolveGitHubRepo(fuzzy: ExtensionAPI, cwd: string): Promise<RepoResolution> {
	const result = await fuzzy.exec("git", ["remote", "-v"], { cwd, timeout: 5_000 });
	if (result.code !== 0) {
...
export default function (fuzzy: ExtensionAPI): void {
	fuzzy.on("session_start", async (_event, ctx) => {
		const resolvedRepo = await resolveGitHubRepo(fuzzy, ctx.cwd);
		if (!resolvedRepo.ok) {
