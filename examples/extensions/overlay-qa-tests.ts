 ...
 *
 * Usage: fuzzy --extension ./examples/extensions/overlay-qa-tests.ts
 *
 * Commands:
...
import type { ExtensionAPI, ExtensionCommandContext, Theme } from "@fuzzyos/fuzzy-code";
import type { Component, OverlayAnchor, OverlayHandle, OverlayOptions, TUI } from "@fuzzyos/fuzzy-tui";
import { Input, matchesKey, truncateToWidth, visibleWidth } from "@fuzzyos/fuzzy-tui";
...
export default function (fuzzy: ExtensionAPI) {
	// Animation demo - proves overlays can handle real-time updates (like fuzzy-doom would need)
	fuzzy.registerCommand("overlay-animation", {
	...
	fuzzy.registerCommand("overlay-anchors", {
	...
	fuzzy.registerCommand("overlay-margins", {
	...
	fuzzy.registerCommand("overlay-stack", {
	...
	fuzzy.registerCommand("overlay-overflow", {
	...
	fuzzy.registerCommand("overlay-edge", {
	...
	fuzzy.registerCommand("overlay-percent", {
	...
	// Test maxHeight
	fuzzy.registerCommand("overlay-maxheight", {
	...
	fuzzy.registerCommand("overlay-sidepanel", {
	...
	fuzzy.registerCommand("overlay-toggle", {
	...
	fuzzy.registerCommand("overlay-passive", {
	...
	fuzzy.registerCommand("overlay-focus", {
	...
	fuzzy.registerCommand("overlay-streaming", {
	...
	private startProcess(): void {
		...
		this.proc = spawn("bash", [
			...
			for i in $(seq 1 100); do
				...
				DIR="/Users/nicobailon/Documents/development/fuzzyos/packages/fuzzy-code/src/modes/interactive"
				...
				if [ $((i % 5)) -eq 0 ]; then
					echo -e "  \\033[32m✓ Successfully processed \${i} files in /Users/nicobailon/Documents/development/fuzzyos\\033[0m"
				fi
				if [ $((i % 7)) -eq 0 ]; then
...
// Animation demo - proves overlays can handle real-time updates like fuzzy-doom
class AnimationDemoComponent extends BaseOverlay {
	...
	render(width: number): string[] {
		...
		lines.push(border("│") + padLine(th.fg("dim", " (fuzzy-doom uses same approach)")) + border("│"));
		lines.push(border("│") + padLine(``) + border("│"));
