 ...
 * - ~/.fuzzy/agent/presets.json (global)
 * - <cwd>/.fuzzy/presets.json (project-local)
 *
 ...
 *
 * Usage:
 * - `fuzzy --preset plan` - start with plan preset
...
import type { Api, Model } from "@fuzzyos/fuzzy-ai";
...
export default function presetExtension(fuzzy: ExtensionAPI) {
	...
	async function applyPreset(name: string, preset: Preset, ctx: ExtensionContext): Promise<boolean> {
		...
		if (activePresetName === undefined) {
			originalState = {
				model: ctx.model,
				thinkingLevel: fuzzy.getThinkingLevel(),
				tools: fuzzy.getActiveTools(),
			};
		}
		...
		if (preset.thinkingLevel) {
			fuzzy.setThinkingLevel(preset.thinkingLevel);
		}

		// Apply tools if specified
		if (preset.tools && preset.tools.length > 0) {
			const allToolNames = fuzzy.getAllTools().map((t) => t.name);
			...
			if (validTools.length > 0) {
				fuzzy.setActiveTools(validTools);
			}
		}
		...
		if (result === "(none)") {
			...
			if (originalState) {
				if (originalState.model) {
					await fuzzy.setModel(originalState.model);
				}
				fuzzy.setThinkingLevel(originalState.thinkingLevel);
				fuzzy.setActiveTools(originalState.tools);
			} else {
				fuzzy.setActiveTools(["read", "bash", "edit", "write"]);
			}
	...
	fuzzy.registerShortcut(Key.ctrlShift("u"), {
	...
	fuzzy.on("before_agent_start", async (event) => {
	...
	fuzzy.on("session_start", async (_event, ctx) => {
		...
		const presetFlag = fuzzy.getFlag("preset");
	...
	fuzzy.on("turn_start", async () => {
		if (activePresetName) {
			fuzzy.appendEntry("preset-state", { name: activePresetName });
		}
	});
}
